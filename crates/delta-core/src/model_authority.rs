...
//! atomic write. The consolidated authority is the sole source of truth.
...
impl ModelAuthority {
    ...
    pub fn set_provider(
        ...
        self.add_model_pref(prefs, &recommended);
        let current = prefs
            .get("default_model")
            .and_then(Value::as_str)
            .unwrap_or("");
    ...
    pub fn remove_provider(&self, name: &str) -> Result<Value, String> {
        ...
        if custom {
            ...
            retain_unprefixed_strings(prefs, "models", &prefix);
            retain_unprefixed_strings(prefs, "hidden_models", &prefix);
        }
        self.write_authority(&file)?;
    ...
    pub fn set_default_model(&self, model: &str) -> Result<Value, String> {
        ...
        let mut file = self.read_authority()?;
        self.add_model_pref(&mut file.prefs, model);
    ...
    pub fn fetch_models(&self, name: &str, fields: &Value) -> Result<Value, String> {
        ...
        for model in models.iter().filter_map(Value::as_str) {
            ...
            self.add_model_pref(&mut file.prefs, &full);
            if !before {
                added.push(full);
            }
        }
    ...
    fn add_model_pref(&self, prefs: &mut Map<String, Value>, model: &str) {
...
fn probe_provider(config: &ProbeConfig, should_include_models: bool) -> Value {
    let include_models = should_include_models;
...
fn openai_fields(base_url: &str, is_official: bool) -> Value {
    let official = is_official;
    let mut fields = vec![
        json!({
...
fn validate_profile(
    descriptor: &ProviderDescriptor,
    profile: &Map<String, Value>,
    should_require_complete: bool,
) -> Result<(), String> {
    let require_complete = should_require_complete;
    if require_complete
        && profile
            .get("api_key")
            .and_then(Value::as_str)
...
fn retain_unprefixed_strings(root: &mut Map<String, Value>, key: &str, prefix: &str) {
    if let Some(values) = root.get_mut(key).and_then(Value::as_array_mut) {
