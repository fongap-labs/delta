...
fn parse_run_status(data: &str) -> String {
...
impl TaskStore {
    ...
    pub fn complete_run(
        ...
        let max_runs = task_json
            ...
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let last_status = parse_run_status(run_data);
...
mod tests {
    ...
    fn get_run_task() {
        ...
        let task = store.get_run_task("__run__run_1").unwrap();
        assert!(task.is_some());
        assert_eq!(task.unwrap().id, "task_1");

        // Non-run session
        assert!(store.get_run_task("other").unwrap().is_none());
        // Non-existent run
        assert!(store.get_run_task("__run__nope").unwrap().is_none());
    }
