...
//!
//! Multiplexed Delta Core wire protocol.
//!
...
//! ```
//!
//! Streaming: a ``start`` frame, zero or more ``delta`` frames,
...
//! ```
//!
//! Cancellation: ``request.cancel`` carries ``target_request_id``.
...
//!
//! Concurrency: streaming commands run in a spawned thread so the
...
enum Command {
    ...
    /// register one artifact. Appends `artifact.registered`
    ...
    /// register (or re-register) a source. Returns the SourceRecord.
    ...
    /// get the latest source for a location+origin.
    ...
    /// batch refresh source status from Python observations.
    ...
    /// validate a citation against a source from the ledger.
    ...
    /// evaluate the deterministic completion contract in Rust.
    ...
    /// register validation criteria + result. Appends `validation.registered` to ledger.
    ...
    /// evaluate criteria against artifacts and persist the result.
    ...
    /// register a checkpoint (recovery snapshot).
    ...
    /// list checkpoints, optionally filtered by run_id / session_id.
    ...
    /// close the checkpoint DB handle in the cache.
    #[serde(rename = "checkpoint.close")]
    CheckpointClose { db: String },
    /// ADR-030: evaluate tool call policy (classify + enforce slices).
    ...
    /// ADR-030: classify a tool call into a risk level (L0–L4).
    ...
    /// ADR-035: decide a tool call's execution disposition and transition
    ...
    /// ADR-037 / ADR-038: decide the lifecycle consequence of
    ...
    /// ADR-039: classify a provider failure for retry policy.
    ...
    /// ADR-031: record an approval audit event.
    ...
    /// ADR-047: test command that exercises the streaming ABI.
    ...
    /// Start a new turn (streaming). Emits runtime events on stdout.
    ...
    /// Resume a suspended turn (streaming).
    #[serde(rename = "runtime.resume")]
    RuntimeResume { session_id: String },
    /// Retry the failed turn (streaming).
    ...
    /// Truncate messages from index (revert).
    ...
    /// Non-streaming provider completion.
    ...
    /// Streaming provider completion (SSE).
...
fn event_to_json(ev: delta_core::LedgerEvent) -> Value {
    serde_json::json!({
        "run_id": ev.run_id,
        "seq": ev.seq,
        "type": ev.r#type,
...
/// Stream context: carries request_id, cancel flag, and shared stdout.
struct StreamCtx {
    request_id: u64,
...
/// Helper: write one JSON line to the shared stdout.
...
fn handle_stream(cmd: Command, ctx: StreamCtx) {
    ...
    match cmd {
        ...
        Command::ProviderStream {
            ...
            match delta_core::provider::stream(&req, &mut writer, &request_id.to_string(), &cancel)
            {
                Ok(result) => {
...
fn handle(cmd: Command, cache: &Mutex<ConnCache>) -> Value {
    ...
    let result: Result<Value, String> = match cmd {
        ...
        Command::TaskForRunSession { db, session_id } => {
            ...
            match store.get_run_task(&session_id) {
                Ok(Some(entry)) => {
                    let mut val = entry.data.clone();
                    if let Some(obj) = val.as_object_mut() {
                        obj.insert("enabled".into(), Value::Bool(entry.enabled));
        ...
        Command::PolicyEvaluate {
            ...
            let dec = match serde_json::from_value::<delta_core::Decision>(decision) {
                Ok(d) => d,
                Err(e) => return err(e.to_string()),
            };
            let root_entries: Vec<delta_core::RootEntry> = roots
                .into_iter()
            ...
            let input = PolicyEvaluateInput {
                ...
                workspace_root,
                roots: root_entries,
                mode: delta_core::ExecutionMode::Execute,
            };
            match delta_core::evaluate(input) {
        ...
        Command::ToolLifecyclePlan {
            ...
            match delta_core::plan_tool_lifecycle(writer, &input) {
                Ok(output) => Ok(serde_json::to_value(output).unwrap()),
                Err(e) => Err(e.to_string()),
            }
        }
        Command::ToolLifecycleCancel {
            ...
            match delta_core::cancel_tool_lifecycle(writer, &input) {
                Ok(output) => Ok(serde_json::to_value(output).unwrap()),
                Err(e) => Err(e.to_string()),
            }
        }
        Command::RetryClassify {
            ...
            Ok(serde_json::to_value(delta_core::classify_error(&input)).unwrap())
        }
        ...
        Command::ProviderComplete {
            ...
            match delta_core::provider::complete(&req) {
                Ok(v) => Ok(v),
                Err(e) => Err(e),
            }
        }
...
/// Session registry — holds cloneable active handles keyed by session_id.
...
fn main() -> std::process::ExitCode {
    ...
    // Active stream registry — maps request_id → cancel flag.
    ...
    // Non-streaming commands continue through the standard dispatch path.
    ...
    for line in stdin.lock().lines() {
        ...
        if cmd.is_streaming() {
            // Dispatch streaming commands to a background thread so
