...
fn band_level(
    ...
    let requires_approval = is_approval_required;
    match risk {
        "high" => {
...
pub fn enforce_scope(
    ...
    for target in targets {
        ...
        let mut is_under_any = false;
        let mut is_under_writable = false;
        for (canon_root, writable) in &canon_roots {
            if canon_parent.starts_with(canon_root) {
                is_under_any = true;
                if *writable {
                    is_under_writable = true;
                }
            }
        }

        if is_under_writable {
            continue;
        }

        let detail = if is_under_any {
...
mod tests {
    ...
    #[test]
    fn enforce_scope_allows_is_under_writable_root() {
        let mut decision = sample_decision();
    ...
    // ------------------------------------------------------------------
    // Plan Mode Trust hard constraint.
    // ------------------------------------------------------------------
