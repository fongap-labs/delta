...
impl McpStore {
    ...
    pub fn configs(&self) -> BTreeMap<String, Value> {
...
fn redact_value(value: &Value, parent_key: Option<&str>) -> Value {
    match value {
        Value::Object(object) => {
            let redact_all_children = parent_key.is_some_and(is_secret_group);
        ...
        _ if parent_key.is_some_and(is_secret_key) => Value::String(REDACTED.to_string()),
        _ => value.clone(),
    }
}

fn is_secret_group(key: &str) -> bool {
