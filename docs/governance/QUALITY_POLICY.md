...
新增、删除或重命名 CI job 时必须同步检查 `validate-merge` 的依赖关系。
...
- Rust Authority 域无 Python fallback writer；
- 无新增 persistent Python application authority；
- portable / release smoke。

架构验收关注**行为和 Authority**，不关注 Rust 百分比或 Python 文件数。
...
测试应优先覆盖稳定 Runtime / Capability / IPC contract，而不是绑定内部 class 或 endpoint 实现。
