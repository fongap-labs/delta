...
本文规定 Delta 的依赖升级、Dependabot、兼容性检查、自动合并，以及核心产品依赖与 Worker 生态依赖的边界。
...
删除 Python core dependency 时，应确认 `uv.lock` 中只剩仍被 Worker / dev / packaging 真实需要的 Python 依赖。
...
不再需要的 Python sidecar packaging 依赖应物理删除，而不是永久留作备用。
