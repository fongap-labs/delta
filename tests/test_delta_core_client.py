...
DEV_BINARIES = (
    REPO_ROOT / "crates" / "delta-core" / "target" / "release" / TARGET,
    REPO_ROOT / "crates" / "delta-core" / "target" / "debug" / TARGET,
)
...
def test_default_lookup_finds_dev_build():
    ...
    build at ``crates/delta-core/target/{release,debug}/``.

    Priority: release > debug. If neither exists, returns None."""
    import os as _os
    from packages.delta_core_client import find_core_binary

    _os.environ.pop("DELTA_CORE_BINARY", None)
    found = find_core_binary()
    dev = _dev_binary()
    if dev is None:
...
def test_command_timeout_raises_delta_core_error(monkeypatch, tmp_path):
    ...
    responses are routed by the background reader thread, so
