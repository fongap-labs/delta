...
Multiplexed Runtime Protocol.

A dedicated stdout reader thread and per-request routing allow concurrent requests:
...
CRATE_DIR = REPO_ROOT / "crates" / "delta-core"
...
# ``crates/delta-core/src/bin/delta_core.rs``.
...
class DeltaCoreClient:
    """Persistent multiplexed client to the delta_core Rust process.
    ...
    * ``_crash()`` — fans out a crash error to all pending requests and
    ...
    def __init__(
        ...
        if binary_path is None:
            binary_path = find_core_binary()
            if binary_path is None:
                raise DeltaCoreError(
                    "delta_core binary not found in any standard location; "
                    "set DELTA_CORE_BINARY or build crates/delta-core"
                )
        self._binary_path = binary_path
        self._command_timeout = command_timeout

        # bounded inflight requests — admission control.
        ...
        # RunningTask registry — who spawned owns completion.
    ...
    def _acquire_inflight(self) -> None:
        """admission control. Acquire one inflight slot.
    ...
    def command(self, payload: dict[str, Any]) -> Any:
        ...
        bounded inflight — raises ``DeltaCoreError("overload")`` if
        ...
        try:
            ...
            is_registered = False
            with self._running_lock:
                is_registered = request_id in self._running
