...
Background tasks (`run_shell` with `should_run_background`) get their own process — NOT the
...
- **managed** (default): tracked here and killed by `shutdown()` when the session's engine
  ...
  calls mid-session).
- **detached durable** (`should_detach=true`, explicit opt-in): behaves like today's immortal
...
class Executor(ABC):
    ...
    def run_background(self, command: str, should_detach: bool = False) -> dict[str, Any]:
...
class _BackgroundTask:
    ...
    def __init__(
        ...
        self.has_head_drop = False  # a read skipped lines that already fell off
        self._cursor = 0
    ...
    def get_output(self) -> tuple[str, bool]:
        """Output appended since the last read. Returns ``(text, has_head_drop)`` —
        ``has_head_drop`` is True when unread lines fell off the bounded buffer's front."""
        with self._lock:
            start = max(self._cursor - self._base, 0)
            has_head_drop = self._cursor < self._base
            new = "".join(list(self._lines)[start:])
            self._cursor = self._base + len(self._lines)
        return new, has_head_drop
...
class LocalExecutor(Executor):
    ...
    def _spawn(self) -> None:
        ...
        if self._is_windows:
            ...
            driver = (
                ...
                "Invoke-Expression $__delta_command 2>&1 | "
                "Out-String -Stream -Width 4096 | "
                "ForEach-Object { [Console]::Out.WriteLine($_) }; "
                "$__delta_success = $?; "
                "$__delta_exit = if ($LASTEXITCODE -ne 0) { [int]$LASTEXITCODE } "
                "elseif ($__delta_success) { 0 } else { 1 }; "
                "} catch { "
                "[Console]::Error.WriteLine($_.Exception.Message); "
                "$__delta_exit = 1; "
                "} "
                f"[Console]::Out.WriteLine('{marker} ' + $__delta_exit + ' ' + $PWD.Path); "
                "}"
            )
            argv = [
                self._shell_path,
    ...
    def run(self, command: str, timeout: float | None = None) -> dict[str, Any]:
        ...
        while True:
            ...
            if self._marker in item:
                parsed_exit = _parse_exit_code(item, self._marker)
        ...
        return self._result(
            ...
            truncated=truncated,
            error="interrupted by user" if is_aborted else None,
        )
            ...
            sink(
                {
                    ...
                    "detach": bool(task.should_detach),
                    **extra,
                }
            )
        except Exception:
        ...
        with self._io_lock:
            ...
            try:
                task = _BackgroundTask(
                    task_id, command, self.cwd, self._env, should_detach=should_detach
                )
    ...
    def background_output(self, task_id: str) -> dict[str, Any]:
        ...
        output, head_dropped = task.get_output()
        truncated = len(output) > self.max_output_chars
        if truncated:
            output = output[-self.max_output_chars :]
    ...
    def shutdown(self) -> None:
        ...
        process tree), then close the shell. Detached (`should_detach=true`) tasks are left
        running. NOT called by `close()` — that runs mid-session on the timeout-recovery
        path, where managed tasks must survive."""
        with self._io_lock:
            managed = [t for t in self._bg_tasks.values() if not t.should_detach]
        for task in managed:
...
_RUN_SHELL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "are). Set should_run_background for long-running processes like dev servers, then "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "should_detach": {
                    "type": "boolean",
                    "description": (
                        "Only with should_run_background: keep the process alive even after "
...
_TASK_OUTPUT_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "run_shell should_run_background=true, plus its status and exit code."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
_TASK_KILL_SCHEMA = {
    ...
    "function": {
        ...
        "description": "Stop a background task started with run_shell should_run_background=true.",
        "parameters": {
            "type": "object",
            "properties": {
                "task_id": {
...
def shell_tools(executor: Executor) -> list:
    ...
    def run_shell(
        ...
        should_run_background: bool = False,
        should_detach: bool = False,
    ) -> dict:
        ...
        if should_run_background:
            return executor.run_background(command, should_detach=should_detach)
        timeout = None
