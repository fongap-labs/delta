...
plus `is_multi_select` for choose-several. The Rust Runtime handles the typed response lifecycle: the
...
def normalize_questions(raw) -> list[dict]:
    ...
    {question, header, options: [canonical option], can_enter_text, is_multi_select}."""
    out: list[dict] = []
        ...
        out.append(
            {
                ...
                "can_enter_text": bool(entry.get("can_enter_text", True)),
                "is_multi_select": bool(entry.get("is_multi_select", False)),
            }
        )
    return out
...
def question_item_fields(args: dict) -> dict | None:
    ...
    if grouped:
        ...
        return {
            ...
            "can_enter_text": first["can_enter_text"],
            "is_multi_select": first["is_multi_select"],
            "header": first["header"],
            "questions": grouped,
        }
    ...
    return {
        ...
        "can_enter_text": bool(args.get("can_enter_text", True)),
