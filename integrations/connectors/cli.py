...
python -m integrations.connectors.cli status
    Show which platforms are configured (token present) + allowlist size.

python -m integrations.connectors.cli fake [--user U1] [--allow U1]
...
python -m integrations.connectors.cli send --target telegram:12345 --text "hi"
...
def _cmd_send(target: str, text: str) -> int:
    tool = build_send_message(SecretStore())
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="delta-connectors")
