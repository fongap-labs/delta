...
def _meta(name: str, *, is_approval_required: bool, capabilities: list[str]):
...
def _attach(
    ...
    *,
    ...
    attach_tool_metadata(
        fn,
        schema=schema,
        metadata=_meta(name, is_approval_required=is_approval_required, capabilities=caps),
    )
    ...
    *,
    ...
    def email_search(
        ...
        criteria, crit_err = build_search_criteria(
            ...
            since=since,
            before=before,
            is_unread_only=bool(is_unread_only),
        )
        if criteria is None:
