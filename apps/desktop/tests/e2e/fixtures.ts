...
function commandSources(): Record<string, string> {
  return {
    ...
    update_context_bar: `(args) => { $state.settings.context_bar = !!args.isShown; return { ok: true, context_bar: $state.settings.context_bar }; }`,
    update_pdf_settings: `(args) => { const p = args.patch || {}; Object.assign($state.settings, p); return { ok: true }; }`,
    update_compaction: `(args) => { const p = args.patch || {}; Object.assign($state.settings, p); return { ok: true }; }`,
    settings_set_language: `(args) => { $state.settings.language = args.language; return { ok: true, language: args.language }; }`,
    settings_set_onboarded: `(args) => { $state.settings.onboarded = !!args.isOnboarded; return { ok: true, onboarded: $state.settings.onboarded }; }`,
    update_session_peek: `(args) => { $state.settings.sessions_peek = args.count; return { ok: true }; }`,
    update_scratch_base: `(args) => { $state.settings.scratch_base = args.path; return { ok: true }; }`,
    settings_set_nav_layout: `(args) => { $state.settings.nav_layout = args.layout; return { ok: true }; }`,
    update_model_key: `(args) => { $state.settings.has_key = true; $state.settings.model_ready = true; return { ok: true }; }`,
    update_model_default: `(args) => { $state.settings.model = args.modelId; return { ok: true, model: args.modelId }; }`,
    ...
    update_session_flags: `(args) => { const s = $state.sessions.find((x) => x.session_id === args.sessionId); if (s) { if (typeof args.isPinned === "boolean") s.pinned = args.isPinned; if (typeof args.isArchived === "boolean") s.archived = args.isArchived; } return { ok: true }; }`,
    ...
    update_session_reasoning: `(args) => { const s = $state.sessions.find((x) => x.session_id === args.sessionId); if (s) s.reasoning_effort = args.effort; return { ok: true, reasoning_effort: args.effort }; }`,
    session_revert: `(args) => ({ ok: true })`,
    workspaces_recent: `(args) => ({ workspaces: [] })`,
    workspaces_trusted: `(args) => ({ workspaces: [] })`,
    update_workspace_trust: `(args) => ({ ok: true, path: args.path, trusted: !!args.isTrusted })`,
    workspace_open: `(args) => ({ ok: true, path: args.path, git_branch: "main" })`,
    session_roots: `(args) => ({ roots: $state.roots })`,
    add_session_root: `(args) => { const existing = $state.roots.find((r) => r.path === args.path); if (existing) existing.writable = !!args.canWrite; else $state.roots.push({ path: args.path, writable: !!args.canWrite, label: (args.path.split("/").filter(Boolean).pop() || args.path), primary: false, exists: true }); return { ok: true, roots: $state.roots }; }`,
    delete_session_root: `(args) => { $state.roots = $state.roots.filter((r) => r.path !== args.path || r.primary); return { ok: true, roots: $state.roots }; }`,
    get_session_unattended: `(args) => ({ ok: true, unattended: !!$state.unattended[args.sessionId] })`,
    update_session_unattended: `(args) => { $state.unattended[args.sessionId] = !!args.isUnattended; return { ok: true, unattended: !!args.isUnattended }; }`,
    session_connections: `(args) => $state.connections`,
    update_session_connection: `(args) => { const row = $state.connections.connected.find((c) => c.connector === args.connector); if (row) row.enabled = !!args.isEnabled; return { ok: true }; }`,
    ...
    list_inbox_routes: `(args) => ({ bindings: $state.inboxRouting })`,
    update_inbox_routes: `(args) => { const row = $state.inboxRouting.find((x) => x.name === args.name); if (row) { row.channel = args.channel; row.target = args.target; } return { ok: true }; }`,
    ...
    clear_memory: `(args) => ({ ok: true })`,
    memory_settings: `(args) => ({ enabled: true, user_rules: "" })`,
    update_memory_settings: `(args) => ({ ok: true })`,
    ...
    resolve_skill_folder: `(args) => ({ ok: false, error: "unknown" })`,
    stage_skill_upload: `(args) => { $state.stagedSkill = { token: "stage-1", name: "greet", description: "says hello", instructions: "Say hello warmly.", files: ["notes.txt"] }; return { ok: true, ...$state.stagedSkill }; }`,
    confirm_skill_upload: `(args) => { const st = $state.stagedSkill; if (!st || args.token !== st.token) return { ok: false, error: "expired" }; $state.skills.push({ name: st.name, description: st.description, instructions: st.instructions, scope: "global", source: "uploaded", enabled: true, path: "/state/skills/" + st.name, files: st.files.length }); $state.stagedSkill = null; return { ok: true }; }`,
    ...
    update_session_skill: `(args) => ({ ok: true })`,
  };
}
...
function fakeAgentSources(): Record<string, string> {
  return {
    ...
    get_autostart: `(args) => false`,
    set_autostart: `(args) => !!args.isEnabled`,
    get_keep_awake: `(args) => false`,
    set_keep_awake: `(args) => !!args.isEnabled`,
    start_window_drag: `(args) => true`,
    set_native_theme: `(args) => true`,
    follow_system_theme: `(args) => true`,
    check_for_update: `(args) => null`,
    download_update: `(args) => null`,
    clear_pending_update: `(args) => null`,
    install_update: `(args) => null`,
    get_dictation_status: `(args) => ({ recording: false, model_installed: false, model_verified: false, test_passed: false, download_in_progress: false, model_name: "", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    dictation_level: `(args) => 0`,
    start_dictation: `(args) => ({ recording: true, model_installed: true, model_verified: true, test_passed: true, download_in_progress: false, model_name: "mock", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    stop_dictation: `(args) => ""`,
    cancel_dictation: `(args) => null`,
    download_dictation_model: `(args) => ({ recording: false, model_installed: true, model_verified: false, test_passed: false, download_in_progress: false, model_name: "mock", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    cancel_dictation_model_download: `(args) => null`,
    verify_dictation_model: `(args) => ({ recording: false, model_installed: true, model_verified: true, test_passed: false, download_in_progress: false, model_name: "mock", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    mark_dictation_test_passed: `(args) => ({ recording: false, model_installed: true, model_verified: true, test_passed: true, download_in_progress: false, model_name: "mock", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    delete_dictation_model: `(args) => ({ recording: false, model_installed: false, model_verified: false, test_passed: false, download_in_progress: false, model_name: "", model_bytes: 0, supported: true, device_summary: "E2E mock", compatibility_reason: null })`,
    runtime_set_mode: `(args) => { const s = $state.sessions.find((x) => x.session_id === args.sessionId); if (s) s.mode = args.mode; return { ok: true, mode: args.mode }; }`,
    resolve_directory_request: `(args) => ({ ok: true })`,
    resolve_plan_request: `(args) => ({ ok: true })`,
    resolve_question_request: `(args) => ({ ok: true })`,
    get_dm_route: `(args) => ({ dm_session: $state.dmSession || null })`,
    update_dm_route: `(args) => { $state.dmSession = args.sessionId; return { ok: true, dm_session: $state.dmSession }; }`,
    browser_state: `(args) => ({ ok: true, available: true, open: false, url: "", title: "", status: "closed", last_action: "", last_result: "", last_error: "", screenshot_data_url: "", updated_at: null, controls: [] })`,
    browser_screenshot: `(args) => ({ ok: true, available: true, open: false, screenshot_data_url: "" })`,
    browser_close: `(args) => ({ ok: true, available: true, open: false, status: "closed" })`,
    ...
    update_connector_tools: `(args) => ({ ok: true })`,
