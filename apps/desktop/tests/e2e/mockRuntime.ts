...
function coreSourceStr(
  ...
  // v8 connector UI contract compatibility for the long-lived E2E seed. Production
  // receives this metadata from Rust after validating Suite's catalog manifest; the
  // browser harness has no Rust catalog loader, so normalize only the test seed here.
  // This mapping never enters the product bundle or runtime authority.
  const connectorUiDetail: Record<string, string> = {
    slack: "workspace_chat",
    gmail: "mail_accounts_privacy",
    google_calendar: "calendar_accounts",
    hubspot: "crm_portals_privacy",
    github: "code_host_installations",
    notion: "accounts",
    attio: "accounts",
    posthog: "accounts",
    mixpanel: "accounts",
  ...
  if (Array.isArray($state.connectors)) {
    $state.connectors = $state.connectors.map((connector: Record<string, any>) => ({
      ...connector,
      ui: connector.ui ?? { detail: connectorUiDetail[connector.name] ?? "generic" },
    }));
  }
