...
export function Onboarding({ onDone }: { onDone: (next?: "work" | "automations") => void }) {
  ...
  const [shouldSkipConfirm, setSkipConfirm] = useState(false);
  ...
  const nextFromForm = !!ps.sel && ps.isDirty && ps.secretFilled;
  const canNext = anyReady || nextFromForm;

  const advance = async () => {
    if (nextFromForm && !ps.credentialed) {
      ps.cancelBackTimer();
      if (!(await ps.testAndSave())) return;
    }
    setStep(1);
  };
