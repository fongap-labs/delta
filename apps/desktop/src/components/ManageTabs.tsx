            ...
            <div className="flex items-center justify-between gap-3 mb-1.5">
              ...
              {info.custom && (
                <button
                  ...
                  disabled={ps.isFetching}
                  data-testid="set-fetch"
                >
                  {ps.isFetching ? "…" : t("providers.refreshModels", undefined, "Refresh models")}
                </button>
              )}
            </div>
...
function DangerZone({
  ...
  const { t } = useI18n();
  const [isArming, setArming] = useState(false);
    ...
    <div className={CARD + " p-3.5"}>
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          disabled={isBusy}
        >
          {isBusy ? "…" : tools ? t("connectors.hideTools") : t("connectors.tools")}
        </button>
...
export function ConnectorTools({ c, onChanged }: { c: Connector; onChanged: () => void }) {
  const { t } = useI18n();
  const toggle = async (toolName: string, isEnabled: boolean) => {
    await updateConnectorTools(c.name, { [toolName]: isEnabled });
    onChanged();
  };
  if (!c.tools?.length)
    return (
...
export function ConnectSetup({
  ...
  const [isBusy, setBusy] = useState(false);
  const [isWaiting, setWaiting] = useState(false); // MCP flow: browser is open
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <div className="border-t border-line px-3.5 py-3 space-y-3">
      {c.mcp && !manualOnly && (
        /* MCP-backed one-click: local OAuth, no cloud account needed. */
        <div className="space-y-2" data-testid="mcp-connect">
          <button className={BTN_ACCENT} onClick={mcpOneClick} disabled={isWaiting}>
            {isWaiting
      ...
      <div>
        <button className={BTN_ACCENT} onClick={submit} disabled={isBusy}>
          {isBusy ? t("connectors.validating") : t("connectors.connect")}
        </button>
      </div>
      {error && <div className="text-[12.5px] text-danger">{error}</div>}
    </div>
