...
// sign-in pane), channel-by-name, day × time, and the §25 hasConsent line for write recipes.
...
interface QuickTemplate {
  ...
  hasConsent?: boolean; // write recipes carry the §25 hasConsent line; reads carry disclosure
...
export function AutomationQuickstart({
  ...
  // is being configured (connects land out-of-band).
  ...
  // §25 hasConsent line shows the HUMAN name (owner catch 2026-07-14: it echoed the raw
  ...
  const create = () => {
    ...
    onCreate({
      ...
      permissions:
        picked.hasConsent && hasConsent && channel
              ...
              {picked.hasConsent ? (
                <label className="flex items-start gap-2.5 mt-3.5 text-[12.5px] text-muted select-none">
                  <input
                    ...
                    data-testid="ob-hasConsent"
                  />
                  <span>
                    {t("automation.hasConsent", {
