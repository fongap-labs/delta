...
export function ModelChecklist({
  ...
  const [shouldHideBlocked, setHideBlocked] = useState(false);
  ...
  const tick = async (id: string, isOn: boolean) => {
    if (!isOn && id === defaultModel) {
    ...
    const res = isOn ? await addModel(id) : await removeModel(id);
