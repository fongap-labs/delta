...
export function useProviderSetup(opts?: { onSaved?: () => void }): ProviderSetupState {
  ...
  const [shouldShowEndpoint, setShowEndpoint] = useState(false);
  ...
  const [isProtocolsLoading, setProtocolsLoading] = useState(true);
  ...
  const openNewCustom = () => {
    if (sel) setDrafts((d) => ({ ...d, [sel]: isDirty ? fields : {} }));
    setSel(null);
    setCreating(true);
    setAlias("");
  ...
  const testAndSave = async (): Promise<boolean> => {
    if (!sel) return false;
    ...
    if (isDirty || !info?.configured) await setProvider(sel, fields).catch(() => {});
    ...
    // the timeout would fire its stale closure (isDirty/fields from before the save) and
  ...
  return {
    ...
    savedState: (credentialed && !isDirty) || verify.state === "ok",
...
export function ProviderForm({
  ...
  const methodFields = choice
    ...
    : [];
  if (ps.isCreating) return <CustomCreateForm ps={ps} tp={tp} />;
  if (!sel) return null;
    ...
    <div key={f.key} className="mb-3 last:mb-0">
      ...
      <div className="relative">
        <input
          ...
          type={f.secret && !shouldShowSecret ? "password" : "text"}
          placeholder={f.secret && ps.credentialed && !ps.isDirty ? "••••••••" : f.placeholder}
        ...
        {f.secret && (ps.fields[f.key] || ps.credentialed) && (
          <button
            ...
            aria-label={shouldShowSecret ? t("providers.hideKey", undefined, "Hide key") : t("providers.showKey", undefined, "Show key")}
 ...
 * Rendered by ProviderForm when `ps.isCreating` is true; distinct from the built-in
...
export function CustomCreateForm({ ps, tp, inline = false }: { ps: ProviderSetupState; tp: string; inline?: boolean }) {
  ...
  const busy = ps.verify.state === "testing" || ps.isFetching;
  ...
  return (
    <div className="max-w-[620px]">
      ...
      {!ps.isProtocolsLoading && !ps.protocolsErr && !proto && !inline && (
      ...
      {fieldsAll.filter((f) => f.secret).length === 0 && !ps.isProtocolsLoading && (
        <>
          <div>
      ...
      <div className="mt-4 border-t border-line pt-3" data-testid={`${tp}-create-footer`}>
        ...
        <div className="mt-2.5 flex items-center justify-end gap-2">
          ...
          >
            {ps.isFetching ? "…" : t("providers.fetchModels", undefined, "Fetch models")}
          </button>
          <button
