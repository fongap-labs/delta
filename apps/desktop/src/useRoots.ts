...
export function useRoots(sessionId: string, reloadKey?: number) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("delta:roots-changed", onChanged);
    return () => window.removeEventListener("delta:roots-changed", onChanged);
  }, [sessionId, reload]);
  ...
  const apply = (res: { ok: boolean; error?: string; roots?: RootInfo[] }): boolean => {
    if (res.ok && res.roots) {
      setRoots(res.roots);
      setError("");
      window.dispatchEvent(new CustomEvent("delta:roots-changed", { detail: sessionId }));
      return true;
    }
  ...
  return { roots, isBusy, error, reload, addRoot: add, toggleAccess, removeRoot: remove };
}
