...
export function ConnectorsList({
  ...
  const shown = shouldShowAll || q ? available : available.slice(0, AVAILABLE_FOLD);
  ...
  return (
    <div>
      ...
      {!shouldShowAll && !q && available.length > AVAILABLE_FOLD && (
        <div className={FOOT}>
...
function statusLine(c: Connector, t: T): string {
  ...
  const structured = c as Connector & { ui?: { detail?: string }; portals?: unknown[] };
  const portalCount =
    structured.ui?.detail === "crm_portals_privacy" ? structured.portals?.length ?? 0 : 0;
  if (portalCount > 1) return t("connectors.portalCount", { n: portalCount });
